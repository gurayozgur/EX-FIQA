...
def get_model(name, **kwargs):
    ...
    if name == "r18":
        ...
        from .iresnet_qs import iresnet50 as r50QS
        return r50QS(False, **kwargs)
        ...
        early_exit_block = kwargs.get("early_exit_block", -1)
        from .vit_qs import VisionTransformer
        return VisionTransformer(
            img_size=112, patch_size=9, num_classes=num_features, embed_dim=512, depth=12,
            num_heads=8, drop_path_rate=0.1, norm_layer="ln", mask_ratio=0.1, mode=mode, early_exit_block=early_exit_block)

    elif name == "vit_s_qs_orig":
        num_features = kwargs.get("num_features", 512)
        mode = kwargs.get("mode", "token")
        from .vit_qs_orig import VisionTransformer
        return VisionTransformer(
            img_size=112, patch_size=9, num_classes=num_features, embed_dim=512, depth=12,
